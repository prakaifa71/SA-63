...
type PatientQuery struct {
	...
	// eager-loading edges.
	withPatient *PatientofphysicianQuery
...
func (pq *PatientQuery) QueryPatient() *PatientofphysicianQuery {
...
func (pq *PatientQuery) WithPatient(opts ...func(*PatientofphysicianQuery)) *PatientQuery {
	query := &PatientofphysicianQuery{config: pq.config}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withPatient; query != nil {
		...
		query.Where(predicate.Patientofphysician(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.PatientColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.patient_patient
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "patient_patient" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_patient" returned %v for node %v`, *fk, n.ID)
			}
